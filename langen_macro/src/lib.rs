...
pub fn langen_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(data) = input.data {
        ...
        let mut regex_parser = regex_syntax::ParserBuilder::default();
        ...
        for variant in data.variants {
            if let Some(r) = variant.attrs.iter().find_map(|attr| {
                if attr.path().is_ident("token") {
                    let t: LitStr = attr
                        .parse_args()
                        .expect(&format!("token argument for \"{}\" must be string literal", variant.ident));
                    Some(t)
                } else {
                    None
                }
            }) {
                let regex = regex_parser.build().parse(&r.value()).expect(&format!(
